...
for i = 1 : x(1)
    k(1 , i) = mdampP(x(1) + 1 - i , 2);
end

for i = 1 : x(2)
    k(2 , i) = mdampP(x(1) + x(2) - i + 1 , 2);
end
...
if x(1) == x(2)
    figure(2)
    plot(mdampP(1 : x(1) , 1) , k(1 , :) , 'b' , mdampP((x(1) + 1 : x(1) + x(2)) , 1) , k(2 , :) , 'r');
    xlabel('Amortecimento');
    ylabel('Controladores');
    title('Fronteira de Pareto');
    grid;
    legend('Fronteira 1' , 'Fronteira 2');% , 'NorthWest');   
else
    figure(2)
    plot(mdampP(1 : x(1) , 1) , k(1 , :) , 'b');
    xlabel('Amortecimento');
    ylabel('Controladores');
    title('Fronteira de Pareto');
    grid;
    legend('Fronteira 1');% , 'NorthWest');   
end